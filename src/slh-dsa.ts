 ...
 * Buffer-aliasing contract: `thash1` / `thashN` write into the caller-provided `out` and return
 * a view of it; without `out` they allocate a fresh result, like `PRFaddr`. The batched
 * `*Fill` calls write into per-context scratch, which stays valid until the next batch on the
 * same lane. `clean()` wipes the scratch buffers along with the hash states.
 */
export type Context = {
  /**
