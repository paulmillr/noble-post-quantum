...
async function main() {
  ...
  // jsbt 0.6 renamed the dimension options and takes metrics as a list.
  const prevFile = './benchmark/platforms.json';
  await compare('Post-Quantum platforms', {}, libs, {
    libraryDimensions: ['algorithm', 'platform', 'operation'],
    dimensions: ['algorithm', 'operation', 'platform'],
    patchArgs: (args, obj) => {
    ...
    // Optional: compare against a saved run, when one is present.
    loadRun: existsSync(prevFile) ? prevFile : undefined,
  });
}
